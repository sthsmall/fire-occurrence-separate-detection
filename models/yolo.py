...
# Ensure all layers and input are using float16
model = SwinStage(dim=96, c2=192, depth=2, num_heads=3, window_size=7)
model = model.half()  # Ensure all model parameters are in float16
input_tensor = torch.randn(1, 3, 224, 224).half()  # Ensure input tensor is in float16
